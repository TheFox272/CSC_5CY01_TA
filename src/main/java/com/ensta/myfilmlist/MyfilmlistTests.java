...
public class MyfilmlistTests {
	...
	public void calculerDureeTotaleTest() {
		...
		try {
			int dureeTotale = myFilmsService.calculerDureeTotale(leSeigneurDesAnneaux);
	...
	public void updateRealisateurCelebresTest() {
		...
		try {
			...
			// Attendue : 1
			System.out.println("Combien de realisateurs celebres ? " + realisateursCelebres.size());
			realisateursCelebres.forEach(System.out::println);
		} catch (ServiceException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
